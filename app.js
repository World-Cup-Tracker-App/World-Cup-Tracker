// app.js — FULL UPDATED ROUND ENGINE (24 / 32 / 48 TEAMS)
...
function renderGroupStage(worldcup, container, editable) {
  ...
  // ⭐ FIX #1 — MUST compute standings BEFORE rendering
  ...
  groupOrder.forEach(groupId => {
    const group = groups.find(g => g.id == groupId); // ⭐ FIX #2 — use == not ===
    if (!group) return;
    ...
    groupMatches.forEach(m => {
      ...
      miniTable.className = "group-table mini";
      ...
      groupBox.appendChild(miniTable);
    });

    // ============================
    // FULL GROUP TABLE
    // ============================
    const standings = engine.groups.find(g => g.groupId == group.id); // ⭐ FIX #3
    if (standings) {
